...
it('renders a native branch select on checkout', function () {
    $product = checkoutProduct();
    ...
    $this->get(route('checkout.create'))
        ->assertOk()
        ->assertSee('name="delivery_branch_id"', false)
        ->assertSee('x-model="selectedBranch"', false)
        ->assertDontSee('x-model="branchQuery"', false);
});
